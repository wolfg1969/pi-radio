...
class Player:
    ...
    def __init__(self, uid, uname, token, expire, pipe_file_w):
    ...
    def __call_mpg321(self, inputQueue, outputQueue):
        ...
        while True:
            ...
            self.logger.debug("streaming radio from %s", song_url.replace('\\',''))
	    subprocess.call(["/usr/bin/ffmpeg","-i",song_url.replace('\\', ''),"-f","s16le","-acodec","pcm_s16le","-ac", "1" ,"-ar","44100","-"],stdout=self.pipe_file, stderr=open(os.devnull, "w"))
